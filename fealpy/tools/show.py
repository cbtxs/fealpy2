...
def showmultirate(plot, k, N, errorMatrix, labellist, optionlist=None, lw=1,
        ms=4, propsize=10, computerate=True, variable = None):
    ...
    for i in range(m):
        if len(N.shape) == 1:
            showrate(axes, k, N, errorMatrix[i], optionlist[i],
                     label=labellist[i], lw=lw, ms=ms, computerate=computerate, variable=variable)
        else:
            showrate(axes, k, N[i], errorMatrix[i], optionlist[i], label=labellist[i], lw=lw, ms=ms, computerate=computerate, variable=variable)
    axes.legend(loc=3, framealpha=0.2, fancybox=True, prop={'size': propsize})
    return axes

def showrate(axes, k, N, error, option, label=None, lw=1, ms=4, computerate=True, variable=None):
